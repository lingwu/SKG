(function(ns){
	...
	var AbstractElement = new Class({
		...
		afterRemote : function(json){
			...
			if(!!json.onSuccess){ json.onSuccess(); }
		...
		remoteValidate : function(){
			...
			if(this.isValid && this.isRemote() && this.value != '') {
				this.params = {};
				this.params[this.element.name] = this.value;
				this.params.temppara = Math.random();
		...
		getMessage : function(){
			return (this.$json && this.$json.message ? this.$json.message : this.options[this.isValid ? 'pass' : (this.value === '' ? 'empty' : 'warn')]) || '\28\u672a\u914d\u7f6e\u63d0\u793a\u4fe1\u606f\29';
		},
		trim : function(text){
